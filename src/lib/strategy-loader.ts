...
import { BasicStrategyData, getStrategyData, StrategyAction, StrategyActionTable } from './strategy-registry';
...
export interface OptimizedStrategy {
  ...
  doubleSoft: Map<number, Map<number, StrategyAction>>;
  doubleHard: Map<number, Map<number, StrategyAction>>;
}
...
export function loadBasicStrategy(rules: BlackjackRules): OptimizedStrategy {
  ...
  // Convert hard doubles
  for (const [total, dealerActions] of Object.entries(strategyData.doubleHard)) {
    const totalNum = parseInt(total);
    const dealerMap = new Map<number, StrategyAction>();
    ...
    optimized.doubleHard.set(totalNum, dealerMap);
  }

  // Convert soft doubles
  for (const [total, dealerActions] of Object.entries(strategyData.doubleSoft)) {
    const totalNum = parseInt(total);
    const dealerMap = new Map<number, StrategyAction>();
    ...
    optimized.doubleSoft.set(totalNum, dealerMap);
  }
